...
async function testOpenAIStructuredOutputs() {
  ...
  try {
    const { object } = await generateObject({
      model: openaiProxy('gpt-4o-mini'),
      schema: TestSchema,
      prompt: 'Extract command from "Create an urgent issue" and return as JSON',
    });
    console.log('✅ SUCCESS:', object);
  } catch (error) {
    console.log('❌ FAILED:', (error as Error).message);
  ...
  console.log('\n🔧 Test 3: Manual JSON with response_format');
  try {
    const response = await fetch('https://openai-proxy.air.dev.hioscar.com/v1/chat/completions', {
      method: 'POST',
      headers: {
      ...
      body: JSON.stringify({
        model: 'gpt-4o-mini',
        messages: [
          { role: 'user', content: 'Extract command from "Create urgent issue" as JSON with fields: command, priority, reason' }
        ],
...
async function main() {
  ...
  try {
    ...
    console.log('• generateObject() support depends on the specific implementation');
