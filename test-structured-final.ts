...
async function testJsonExtraction() {
  ...
  try {
    const { text } = await generateText({
      ...
      system: 'You are a JSON generator. Always respond with valid JSON only, no other text.',
...
async function testCommandEnum() {
  ...
  for (const msg of messages) {
    try {
      const { text } = await generateText({
        model: litellm('claude-3-7-sonnet'),
        system: 'Extract commands from messages. Respond only with JSON containing "command" and "confidence" (0-1).',
        prompt: msg,
        maxTokens: 100,
      });
      ...
      try {
        ...
        console.log(`   ✅ ${validated.command} (confidence: ${validated.confidence})`);
      } catch (e) {
...
async function testStreaming() {
  ...
  try {
    const { textStream } = await streamText({
      ...
      prompt: 'Plan 3 tasks for debugging a login issue. Return as JSON array with "task" and "priority" fields.',
      maxTokens: 200,
    });
...
function createCommandParser() {
  ...
  return async (message: string) => {
    const { text } = await generateText({
      model: litellm('claude-3-7-sonnet'),
      system: 'You are a command classifier. Respond with only one word: CREATE_ISSUE, UPDATE_STATUS, QUERY_DATABASE, or NONE.',
      prompt: message,
      maxTokens: 10,
    });
