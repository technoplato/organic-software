...
const CommandEnum = z.enum(['CREATE_ISSUE', 'UPDATE_STATUS', 'QUERY_DATABASE', 'NONE']);
...
async function extractCommand(message: string) {
  const { text } = await generateText({
    ...
    system: 'Extract commands from messages. Respond with JSON only containing: command (CREATE_ISSUE/UPDATE_STATUS/QUERY_DATABASE/NONE), priority (low/medium/high), reason (brief text).',
    prompt: message,
    maxTokens: 150,
  });
...
async function analyzeText(text: string) {
  const { text: response } = await generateText({
    model: litellm('claude-3-7-sonnet'),
    system: 'Analyze text and return JSON with: technical (array), personal (array), command (CREATE_ISSUE/UPDATE_STATUS/QUERY_DATABASE/NONE), summary (one sentence).',
    prompt: text,
    maxTokens: 300,
  });
...
async function classifyCommand(message: string): Promise<z.infer<typeof CommandEnum>> {
  const { text } = await generateText({
    model: litellm('claude-3-7-sonnet'),
    system: 'Classify the message into one command. Respond with only: CREATE_ISSUE, UPDATE_STATUS, QUERY_DATABASE, or NONE',
    prompt: message,
    maxTokens: 10,
  });
...
async function main() {
  ...
  try {
    const cmd1 = await extractCommand("Create an urgent issue about the login bug");
