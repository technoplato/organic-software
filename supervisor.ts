...
const ZSCALER_DISABLE_ONCE_SCRIPT = '/Users/mlustig/dev/tools/zscaler-auto-disabler/disable_once.py';
...
class Supervisor {
  ...
  private spawnProc(spec: ProcSpec) {
    ...
    const p = spawn(spec.cmd, spec.args, { cwd: spec.cwd, stdio: wantPipes ? 'pipe' : 'inherit', env: process.env });
    if (wantPipes) {
  ...
  private async startBundler(detached = false) {
    ...
    if (process.platform === 'darwin' && process.env.SUPERVISOR_OPEN_SIM === '1') {
      extraArgs.push('--ios');
    }
    if (detached) {
  ...
  private isZscalerActive(): boolean {
    try {
      ...
      const result = execSync('netstat -rn | grep -E "165\\.225\\.|104\\.129\\."', { encoding: 'utf8' });
  ...
  private handleZscalerError(): void {
    if (this.zscalerCheckAttempts >= this.maxZscalerAttempts) {
      console.log('⚠️ Max Zscaler disable attempts reached. Please manually disable Zscaler.');
      return;
    }
    ...
    if (this.isZscalerActive()) {
      console.log('🔍 Zscaler detected as active (attempt ' + this.zscalerCheckAttempts + '/' + this.maxZscalerAttempts + ')');
      ...
      console.log('ℹ️ Zscaler appears to be inactive, but fetch is still failing');
...
function main() {
  ...
  if (cmd === 'status') { console.log('Use heartbeats in app to view status'); return; }
  ...
  if (cmd === 'start') sup.start();
    ...
    const send = () => {
      ...
      if (p && p.stdin && !p.killed) {
        try { p.stdin.write('r'); console.log('🔁 Sent reload to Expo bundler'); } catch {}
      } else {
...
function findFreePort(preferred: number[]): Promise<number> {
  return new Promise((resolve) => {
    ...
    const tryNext = () => {
      ...
      const srv = net.createServer();
      srv.once('error', () => { try { srv.close(); } catch {} ; tryNext(); });
      srv.listen(want, () => {
