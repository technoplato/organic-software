...
async function main() {
  ...
  for (let i = 0; i < 10; i++) {
    const res1 = await withTimeout(
      db.queryOnce({
        heartbeats: { $: { where: { id: mobileId }, limit: 1 } },
      }),
      1000,
      "query mobile heartbeat",
    );
  ...
  if (Math.abs(mobile.lastSeenAt - now) > 5000)
    throw new Error("Mobile heartbeat timestamp is stale");
  ...
  for (let i = 0; i < 10; i++) {
    const res2 = await withTimeout(
      db.queryOnce({ heartbeats: { $: { where: { id: hostId }, limit: 1 } } }),
      1000,
      "query host heartbeat",
    );
  ...
  if (Math.abs(host.lastSeenAt - hostNow) > 5000)
    throw new Error("Host heartbeat timestamp is stale");
