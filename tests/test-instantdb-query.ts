  ...
  try {
    ...
    if (result.data.conversations && result.data.conversations.length > 0) {
      ...
      if (sampleConv) {
        ...
        console.log(
          `   • Claude Session ID: ${sampleConv.claudeSessionId || "none"}`,
        );
        console.log(
          `   • Created: ${sampleConv.createdAt ? new Date(sampleConv.createdAt).toLocaleString() : "unknown"}`,
        );
      }
    }
    ...
    if (result.data.messages && result.data.messages.length > 0) {
      ...
      if (sampleMsg) {
        ...
        console.log(
          `   • Content: "${(sampleMsg.content || "").substring(0, 50)}${(sampleMsg.content || "").length > 50 ? "..." : ""}"`,
        );
        console.log(`   • Status: ${sampleMsg.status || "none"}`);
        console.log(
          `   • Timestamp: ${sampleMsg.timestamp ? new Date(sampleMsg.timestamp).toLocaleString() : "unknown"}`,
        );
      }
    }
