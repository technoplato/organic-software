...
async function testLiteLLMEndpoint() {
  ...
  try {
    const { text } = await generateText({
      ...
      prompt:
        'Extract command from "Create an urgent issue" as JSON with fields: command, priority, reason, confidence',
      maxTokens: 150,
    });
...
async function testTTSEndpoint() {
  ...
  const testText =
    "Hello! This is a test of the text-to-speech functionality from the OpenAI proxy endpoint.";
  ...
  for (const model of ttsModels) {
    ...
    try {
      const response = await fetch(
        "https://openai-proxy.air.dev.hioscar.com/v1/audio/speech",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        ...
        console.log(
          `  ✅ ${model.name}: Generated ${audioBuffer.byteLength} bytes`,
        );
        console.log(`  📁 Saved as: ${filename}`);
        console.log(
          `  🎵 Play with: afplay ${filename} (macOS) or mpv ${filename}`,
        );
      } else {
  ...
  for (const voice of voices.slice(0, 3)) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `    ✅ Voice ${voice}: ${audioBuffer.byteLength} bytes → ${filename}`,
        );
      } else {
