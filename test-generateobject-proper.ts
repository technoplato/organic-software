...
async function testOpenAIProxy() {
  ...
  try {
    ...
    console.log('❌ OpenAI Proxy generateObject FAILED:', (error as Error).message);
    console.log('Error type:', (error as Error).constructor.name);
    return false;
  }
}
...
async function testCommandEnumeration() {
  ...
  try {
    const result = await generateObject({
      ...
      prompt: 'Parse this user request into a command: "Please update the user profile with name John and email john@example.com"',
    });
...
async function main() {
  ...
  console.log('\n📊 FINAL RESULTS:');
  console.log('LiteLLM generateObject:', results.litellm ? '✅ WORKS' : '❌ FAILS');
  console.log('OpenAI Proxy generateObject:', results.openaiProxy ? '✅ WORKS' : '❌ FAILS');
  console.log('Command Enumeration:', results.commandEnum ? '✅ WORKS' : '❌ FAILS');
  ...
  if (results.litellm || results.openaiProxy) {
    ...
    console.log('May need to check provider configuration or use fallback methods.');
  }
}
