...
const StreamOfConsciousnessSchema = z.object({
  thoughts: z.object({
    ...
    key_points: z.array(z.string()).describe('Key points extracted from the message'),
    sentiment: z.enum(['positive', 'neutral', 'negative', 'mixed']).describe('Overall sentiment')
  }),
  
  actions: z.object({
    command: CommandEnum.describe('The command to execute based on the message'),
    parameters: z.record(z.any()).optional().describe('Parameters for the command if applicable'),
    priority: z.enum(['low', 'medium', 'high']).describe('Priority level of the action')
  }),
  
  categories: z.object({
    technical: z.array(z.string()).describe('Technical aspects mentioned'),
    personal: z.array(z.string()).describe('Personal or emotional aspects mentioned')
  }),
...
async function testStructuredOutput() {
  ...
  try {
    ...
    if (object.actions.parameters) {
      console.log('  Parameters:', JSON.stringify(object.actions.parameters, null, 2));
    }
