  ...
  for (const modelId of models) {
    ...
    try {
      const response = await fetch(
        `${process.env.LITELLM_BASE_URL || "https://llm-proxy.dev-tools.tools.hioscar.com"}/chat/completions`,
        {
          method: "POST",
    ...
    try {
      const response = await fetch(
        `${process.env.LITELLM_BASE_URL || "https://llm-proxy.dev-tools.tools.hioscar.com"}/models`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const strategies = [
    { name: "no-schema", config: { output: "no-schema" as const } },
    {
      ...
      config: {
        ...
        enum: ["CREATE_ISSUE", "UPDATE_STATUS", "NONE"],
      },
    },
  ];
