...
async function testToolCalling() {
  ...
  const extractCommandTool = tool({
    ...
    parameters: z.object({
      command: z.string().describe('The command: CREATE_ISSUE, UPDATE_STATUS, QUERY_DATABASE, or NONE'),
  ...
  try {
    const { toolCalls } = await generateText({
      ...
      toolChoice: 'required',
      prompt: 'Extract the command from: "Update the status of ticket #123 to done, this is blocking the release!"',
    });
    
    if (toolCalls && toolCalls.length > 0) {
      console.log('✅ Tool calling successful!');
      console.log('Extracted data:', JSON.stringify(toolCalls[0].args, null, 2));
    } else {
...
async function testMultipleCommands() {
  ...
  for (const msg of messages) {
    try {
      ...
      console.log(`📨 "${msg}"`);
      console.log(`   → Command: ${object.command} (confidence: ${object.confidence})`);
    } catch (error) {
...
async function testSystemPrompt() {
  ...
  try {
    const { text } = await generateText({
      model: litellm('claude-3-7-sonnet'),
      system: 'You are a command parser. Respond only with the command name in CAPS.',
...
async function main() {
  ...
  console.log('Using:', process.env.LITELLM_BASE_URL || 'https://llm-proxy.dev-tools.tools.hioscar.com');
