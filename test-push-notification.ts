...
async function getAllPushTokens(): Promise<string[]> {
  try {
    ...
    const tokens = devices
      ...
      .map((device: any) => {
        console.log(`  - ${device.platform}: ${device.pushToken.substring(0, 30)}...`);
      ...
      try {
        ...
        for (const receiptId in receipts) {
          ...
          if (receipt.status === 'ok') {
            console.log(`✅ Notification ${receiptId} was delivered successfully`);
          } else if (receipt.status === 'error') {
            console.error(`❌ Notification ${receiptId} failed: ${(receipt as any).message || 'Unknown error'}`);
