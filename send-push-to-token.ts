...
async function sendNotificationToToken(pushToken: string, title: string, body: string, data?: any) {
      ...
      try {
        ...
        if (receipt) {
          if (receipt.status === 'ok') {
            ...
            console.error(`❌ Notification delivery failed: ${(receipt as any).message || 'Unknown error'}`);
